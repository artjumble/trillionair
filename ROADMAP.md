...
- [ ] Bug: Shortcut buttons appear enabled initially. For example the Firmer Handshake button is enabled until the player gets ~$50 then it switches to disabled. Once the player gets the right amount of money, then button appears enabled with the grow/shrink effect.
